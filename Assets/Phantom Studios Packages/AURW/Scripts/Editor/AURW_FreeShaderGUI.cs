...
namespace aurw
{
    public class AURW_FreeShaderGUI : ShaderGUI
    {
        ...
        // Paleta colores moderna estilo Unity 6 Dark (#242526 base)
        private Color tabBgColor = new Color(0.141f, 0.145f, 0.149f);            // Fondo gris oscuro base (#242526)
        private Color tabActiveColor = new Color(0.3f, 0.52f, 0.82f, 0.3f);         // Azul claro translúcido para tab activo
        private Color sectionBgColor = new Color(0.18f, 0.18f, 0.19f, 0.95f);       // Fondo de secciones, gris oscuro con transparencia
        private Color lineColor = new Color(0.27f, 0.28f, 0.3f, 1f);           // Líneas divisorias gris azulado suave
        private Color labelTextColor = new Color(0.82f, 0.84f, 0.87f);              // Gris claro para textos (#D1D5DB aprox)
        private Color headerColor = new Color(0.87f, 0.89f, 0.93f);              // Títulos un poco más claros, azul grisáceo
        ...
        private void DrawTabSelector()
        {
            ...
            for (int i = 0; i < _tabs.Length; i++)
            {
                ...
                // Fondo tabs: activo azul translúcido, no activo gris oscuro
                EditorGUI.DrawRect(rect, isSelected ? tabActiveColor : tabBgColor);

                // Hover efecto sutil (blanco transparente) solo si no está seleccionado
